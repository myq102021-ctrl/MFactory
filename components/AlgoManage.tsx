...
const ALGO_LIBRARY = [
  { 
    ...
    description: '从多源卫星、无人机或IoT设备获取原始时空数据。该算子支持多种数据源输入，能够自动识别并提取关键特征。',
    params: [
      { label: '提交方式(get/post)', enLabel: 'method', type: 'string', format: 'string', required: true, desc: '提交方式(get/post)', icon: <Globe size={16} /> },
      { label: '请求头', enLabel: 'headers', type: 'map', format: 'header', required: false, desc: '请求头', icon: <Box size={16} /> },
      { label: '提交数据', enLabel: 'body', type: 'string', format: 'body', required: true, desc: 'body', icon: <Activity size={16} /> },
      { label: '数据来源', enLabel: 'source', type: 'select', format: 'string', required: true, desc: '数据来源', icon: <Globe size={16} /> },
      { label: '遥感平台', enLabel: 'platform', type: 'select', format: 'string', required: true, desc: '遥感平台', icon: <Box size={16} /> },
    ] as AlgoParam[]
  },
  { 
    name: '波段合成', 
    ...
    params: [
      { label: '波段范围', enLabel: 'bands', type: 'text', format: 'string', required: true, desc: '示例: 3,2,1表示按B3、B2、B1顺序排列波段', icon: <Pipette size={16} /> },
      { label: '输出坐标系', enLabel: 'crs', type: 'select', format: 'string', required: false, desc: 'EPSG:4326 (WGS 84)', icon: <Globe size={16} /> }
    ] as AlgoParam[]
  },
  { 
    name: '影像镶嵌', 
    ...
    params: [
      { label: '输出坐标系', enLabel: 'crs', type: 'select', format: 'string', required: true, desc: 'EPSG:4326 (WGS 84)', icon: <Globe size={16} /> }
    ] as AlgoParam[]
  },
  { 
    name: '影像裁剪', 
    ...
    params: [
      { label: '裁剪矢量路径', enLabel: 'path', type: 'file', format: 'string', required: true, desc: '支持上传 .shp, .kml, .geojson 格式', icon: <FileJson size={16} /> },
      { label: '输出坐标系', enLabel: 'crs', type: 'select', format: 'string', required: true, desc: 'EPSG:4326 (WGS 84)', icon: <Globe size={16} /> }
    ] as AlgoParam[]
  },
  { 
    ...
    params: [
      { label: '服务名称', enLabel: 'name', type: 'text', format: 'string', required: true, desc: '服务名称', icon: <FileText size={16} /> },
      { label: '输出坐标系', enLabel: 'crs', type: 'select', format: 'string', required: true, desc: 'EPSG:4326 (WGS 84)', icon: <Globe size={16} /> }
    ] as AlgoParam[]
  },
...
export const AlgoManage: React.FC<AlgoManageProps> = ({ onCreateTask }) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white/40 backdrop-blur-xl rounded-2xl shadow-2xl shadow-blue-900/10 border border-white/50 p-4 gap-4 overflow-hidden relative h-full">
      ...
      {/* 算法详情 Modal - 已舍弃，改为 AlgoDetail 组件 */}
    </div>
  );
};
