...
import { DataPlatformSearchModal } from './DataPlatformSearchModal';
...
export const DesignView: React.FC<Props> = ({ lineName, initialData, onBack, onSaveDraft, onSubmit }) => {
  ...
  const [isPlatformSearchOpen, setIsPlatformSearchOpen] = useState(false);
  const [isDataPreviewOpen, setIsDataPreviewOpen] = useState(false);
  const [activeParam, setActiveParam] = useState<string | null>(null);

  // --- Helper cloning functions to avoid JSON circular reference errors ---
  const cloneNodes = (nodesToClone: CanvasNode[]): CanvasNode[] => {
    return nodesToClone.map(node => ({
      id: node.id,
      name: node.name,
      iconKey: node.iconKey || 'Box',
      type: node.type,
  ...
  const cloneConns = (connsToClone: Connection[]): Connection[] => {
    return connsToClone.map(conn => ({
      id: conn.id,
      fromId: conn.fromId,
      toId: conn.toId,
      data: conn.data ? JSON.parse(JSON.stringify(conn.data)) : undefined
    }));
  };
  ...
  const pushHistory = (nodesToSave: CanvasNode[], connsToSave: Connection[]) => {
    setHistory(prev => [...prev, { nodes: cloneNodes(nodesToSave), conns: cloneConns(connsToSave) }]);
    setRedoStack([]); 
  };
  ...
  const handleUndo = () => {
    ...
    setRedoStack(prevStack => [{ nodes: cloneNodes(canvasNodes), conns: cloneConns(connections) }, ...prevStack]);
  ...
  const handleRedo = () => {
    ...
    setHistory(prevHist => [...prevHist, { nodes: cloneNodes(canvasNodes), conns: cloneConns(connections) }]);
  ...
  useEffect(() => {
    if (initialData && initialData.canvasData) {
      ...
      setCanvasNodes(initialData.canvasData.nodes.map(n => ({
        id: n.id,
        name: n.name,
        iconKey: n.iconKey || 'Box',
        type: n.type || 'process',
        pos: n.pos || { x: 300, y: 300 },
  ...
  const onCanvasDrop = (e: React.DragEvent) => {
    ...
    if (originalItem.name === '云盘数据输入' || originalItem.name === '数据输入') {
  ...
  const categories = [
    {
      ...
      items: [
          ...
          { name: '数据输入', icon: <Cloud size={24} className="text-blue-500" />, iconKey: 'Cloud' }
      ]
    },
    {
      title: '处理节点',
    ...
    <div className="flex flex-col h-full bg-[#f8fafc] animate-in fade-in duration-500 overflow-hidden font-['Noto_Sans_SC']">
      <div className="h-[64px] bg-white border-b border-slate-100 flex items-center justify-between px-6 shrink-0 z-30 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            onClick={() => {
              const pipelineData: ProductionLine = {
                id: initialData?.id || `draft-${Date.now()}`,
                name: localLineName || '未命名产线',
                code: initialData?.code || `PL_${Date.now()}`,
                version: initialData?.version || 'V1.0',
                type: initialData?.type || '数据处理产线',
                createTime: initialData?.createTime || new Date().toISOString().replace('T', ' ').slice(0, 19),
                status: '草稿',
                description: initialData?.description || '编排产线草稿',
                canvasData: {
                  nodes: cloneNodes(canvasNodes),
                  connections: cloneConns(connections)
                }
              };
              if (onSaveDraft) onSaveDraft(pipelineData);
              else if (onBack) onBack();
            }}
            className="px-4 h-[36px] flex items-center gap-2 bg-white border border-slate-200 text-slate-600 rounded-lg text-[14px] font-semibold hover:bg-slate-50 transition-colors"
          >
          ...
          <button 
            onClick={() => {
              const pipelineData: ProductionLine = {
                ...
                createTime: initialData?.createTime || new Date().toISOString().replace('T', ' ').slice(0, 19),
                status: '启用',
                description: initialData?.description || '编排产线',
                canvasData: {
                  nodes: cloneNodes(canvasNodes),
                  connections: cloneConns(connections)
                }
              };
              if (onSubmit) onSubmit(pipelineData);
              else if (onBack) onBack();
            }}
            className="px-6 h-[36px] flex items-center gap-2 bg-blue-600 text-white rounded-lg text-[14px] font-bold hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition-all active:scale-95"
          >
            <Send size={16} /> 提交
          </button>
        </div>
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none" style={{ transform: `translate(${canvasOffset.x}px, ${canvasOffset.y}px) scale(${zoom})`, transformOrigin: '0 0' }}>
            ...
            {canvasNodes.map(node => {
              const isCloudInput = node.name.startsWith('云盘数据输入') || node.name.startsWith('数据输入');
              ...
              >
                ...
                `}>
                  <div className={`w-14 h-14 bg-white border border-slate-50 rounded-xl flex items-center justify-center shadow-inner transition-colors ${selectedNodeId === node.id ? 'bg-blue-50/20' : 'group-hover:bg-blue-50/20'}`}>
                    {ICON_MAP[node.iconKey] || node.icon || <Box size={32} className="text-blue-600" />}
                  </div>
        ...
        <div className={`bg-white rounded-xl border border-slate-200 flex flex-col shadow-sm overflow-hidden transition-all duration-300 ${isPropertiesVisible ? 'w-[400px]' : 'w-0 opacity-0 pointer-events-none border-none'}`}>
          {selectedConnection && sourceNodeOfConnection && targetNodeOfConnection ? (
            <ParameterMappingPanel 
              ...
              sourceNode={{ id: sourceNodeOfConnection.id, name: sourceNodeOfConnection.name, icon: ICON_MAP[sourceNodeOfConnection.iconKey] || <Box size={32} className="text-blue-600" /> }}
              targetNode={{ id: targetNodeOfConnection.id, name: targetNodeOfConnection.name, icon: ICON_MAP[targetNodeOfConnection.iconKey] || <Box size={32} className="text-blue-600" /> }}
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100">
                    ...
                    {activeTabs.includes('params') && (
                      <div className="space-y-6 mt-2 pb-6 animate-in fade-in slide-in-from-top-1 px-1">
                        {selectedNode.name.startsWith('云盘数据输入') || selectedNode.name.startsWith('数据输入') ? (
                            <div className="space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                                {/* Radio Buttons for Input Source Selection */}
                                <div className="space-y-2 pb-1 border-b border-slate-100">
                                    <div className="flex items-center gap-5">
                                        <label className="flex items-center gap-2 cursor-pointer text-[13px] font-bold text-slate-700 hover:text-blue-600 transition-colors">
                                            <input 
                                                type="radio" 
                                                name={`inputSource_${selectedNode.id}`}
                                                checked={(selectedNode.params?.inputType || 'cloud') === 'cloud'}
                                                onChange={() => handleParamChange('inputType', 'cloud')}
                                        ...
                                        <label className="flex items-center gap-2 cursor-pointer text-[13px] font-bold text-slate-700 hover:text-blue-600 transition-colors">
                                            <input 
                                                type="radio" 
                                                name={`inputSource_${selectedNode.id}`}
                                                checked={selectedNode.params?.inputType === 'platform'}
                                                onChange={() => handleParamChange('inputType', 'platform')}
                                                className="w-4 h-4 text-blue-600 focus:ring-blue-500 cursor-pointer"
                                            />
                                            <span>从数管平台选择</span>
                                        </label>
                                ...
                                {selectedNode.params?.inputType === 'platform' ? (
                                    <div className="space-y-3">
                                        <div className="flex gap-2">
                                            <input 
                                                type="text" 
                                                placeholder="请选择或输入数管平台数据路径" 
                                                value={selectedNode.params?.inputPath || ''} 
                                                onChange={(e) => handleParamChange('inputPath', e.target.value)}
                                                className="flex-1 h-11 px-4 bg-white border border-slate-200 rounded-lg text-[13px] text-slate-700 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all shadow-sm" 
                                            />
                                            <button 
                                                onClick={() => setIsPlatformSearchOpen(true)}
                                                className="h-11 px-5 bg-blue-600 text-white rounded-lg font-bold text-[13px] hover:bg-blue-700 transition-all shadow-md shadow-blue-500/20 flex items-center gap-1.5 active:scale-95 shrink-0"
                                            >
                                                浏览
                                            </button>
                                        </div>
                                        <div className="flex items-start gap-2 text-[12px] text-blue-500 leading-snug bg-blue-50/40 p-4 rounded-xl border border-slate-100/50">
                                            ...
                                            <span>已选择从数管平台选择，点击“浏览”可打开时空综合检索窗口定位并选取数据。</span>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                ...
                                {/* Matched Resources Card (renders for both cloud and platform) */}
                                  ...
                                  <div className="bg-[#EBF2FF] px-4 py-2.5 border-b border-blue-100 flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      <button 
                                        ...
                                        className="px-2 py-1 hover:bg-blue-100/80 bg-white/60 border border-blue-200/60 rounded-md text-blue-600 transition-all flex items-center gap-1 text-[12px] font-bold shadow-xs active:scale-95"
                                        title="数据预览"
                                      >
                                        <Eye size={13} />
                                        <span>数据预览</span>
      ...
      {activeModal === 'matchedFullscreen' && selectedNode?.params?.matchedFiles && (
       ...
       {/* 数管平台时空检索弹窗 */}
       <DataPlatformSearchModal 
         visible={isPlatformSearchOpen} 
         onClose={() => setIsPlatformSearchOpen(false)} 
         onConfirm={(path, matched) => {
           handleParamChange('inputPath', path);
           if (matched) {
             handleParamChange('matchedFiles', matched);
           }
         }} 
       />
       ...
       {/* 数据预览弹窗 (全屏数字地球 & 数据对) */}
       <DataPreviewModal 
         visible={isDataPreviewOpen} 
         onClose={() => setIsDataPreviewOpen(false)} 
         matchedFiles={selectedNode?.params?.matchedFiles || []} 
       />
    </div>
  );
};
