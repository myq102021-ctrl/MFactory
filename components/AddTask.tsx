...
export const AddTask: React.FC<AddTaskProps> = ({ algoName = '数据采集', onBack, onNavigate }) => {
  ...
  const [inputType, setInputType] = useState<'cloud' | 'platform'>('cloud');
    ...
    <div className="flex-1 flex flex-col bg-white/40 backdrop-blur-xl rounded-2xl shadow-2xl shadow-blue-900/10 border border-white/50 p-4 overflow-hidden h-full font-['Noto_Sans_SC'] relative">
          ...
          {currentStep === 2 && (
            <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in slide-in-from-top-1 duration-500">
               <div className="bg-slate-50/40 p-8 rounded-[24px] border border-slate-100 space-y-5">
                  ...
                  {/* Radio Buttons */}
                  <div className="flex items-center gap-6 ml-28 pb-2 border-b border-slate-100">
                    <label className="flex items-center gap-2 cursor-pointer text-[14px] font-bold text-slate-700 hover:text-blue-600 transition-colors">
                      <input 
                        type="radio" 
                        name="addTaskInputType"
                        checked={inputType === 'cloud'}
                        onChange={() => setInputType('cloud')}
                    ...
                    <label className="flex items-center gap-2 cursor-pointer text-[14px] font-bold text-slate-700 hover:text-blue-600 transition-colors">
                      <input 
                        type="radio" 
                        name="addTaskInputType"
                        checked={inputType === 'platform'}
                        onChange={() => setInputType('platform')}
                        className="w-4 h-4 text-blue-600 focus:ring-blue-500 cursor-pointer"
                      />
                      <span>从数管平台选择</span>
                    </label>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 flex gap-3">
                        <input 
                          type="text" 
                          placeholder={inputType === 'platform' ? "请选择或输入数管平台数据路径" : "请选择或输入输入数据路径"}
                        ...
                        <button 
                          onClick={() => {
                            if (inputType === 'platform') {
                              setIsPlatformSearchOpen(true);
                            } else {
                              setIsInputFileSelectorOpen(true);
                            }
                          }} 
                          className="h-11 px-6 bg-blue-600 text-white rounded-lg font-bold text-[14px] hover:bg-blue-700 transition-all shadow-md shadow-blue-500/20 active:scale-95"
                        >
                          浏览
                        </button>
                        {inputType === 'cloud' && (
                  ...
                  <div className="flex items-start gap-2.5 text-[13px] text-blue-500 bg-blue-50/50 p-4 rounded-xl border border-blue-100/50 leading-relaxed font-medium ml-28">
                    ...
                    <span>
                      {inputType === 'platform' 
                        ? '已选择从数管平台选择，点击“浏览”可打开时空综合检索窗口定位并选取数据。' 
                        : '请选择待处理数据目录并选择卫星配置，系统将自动匹配符合条件的文件。'}
                    </span>
                  </div>
                        ...
                        <div className="bg-[#f8faff] px-6 py-4 border-b border-blue-100 flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-3">
                                <button 
                                  ...
                                  className="px-2.5 py-1 hover:bg-blue-100/80 bg-white border border-blue-200/80 rounded-lg text-blue-600 transition-all flex items-center gap-1.5 text-[12px] font-bold shadow-xs active:scale-95"
                                  title="数据预览"
                                >
                                  <Eye size={15} />
                                  <span>数据预览</span>
      ...
      {activeModal === 'matchedFullscreen' && matchedFiles.length > 0 && (
       ...
       <DataPlatformSearchModal 
         ...
         onConfirm={(path, selectedMatched) => {
           setFormData({ ...formData, inputPath: path });
           if (selectedMatched && selectedMatched.length > 0) {
             setMatchedFiles(selectedMatched);
             setSelectedResultIds(new Set(selectedMatched.map(m => m.id)));
             setHasAttemptedMatch(true);
           }
           setIsPlatformSearchOpen(false);
         }} 
       />
