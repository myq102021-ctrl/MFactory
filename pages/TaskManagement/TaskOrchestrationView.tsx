...
import { DataPlatformSearchModal } from '../../components/DataPlatformSearchModal';
...
export const TaskOrchestrationView: React.FC<Props> = ({ initialPipeline, onBack, onSubmit }) => {
  ...
  const initWorkflow = (pipeline: ProductionLine) => {
    if (pipeline.canvasData && pipeline.canvasData.nodes.length > 0) {
      ...
      const labels = ['开始', '数据输入', '影像裁剪', '服务发布', '结束'];
                      ...
                      {activeTabs.includes('params') && (
                        <div className="space-y-6 mt-2 pb-6 animate-in fade-in slide-in-from-top-1 px-1">
                          {(() => {
                              ...
                              if (baseName.startsWith('云盘数据输入') || baseName.startsWith('数据输入')) {
                                return (
                                  <div className="space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                                      ...
                                      {selectedNode.params?.inputType === 'platform' ? (
                                          <div className="space-y-3">
                                              ...
                                              <div className="flex items-start gap-2 text-[12px] text-blue-500 leading-snug bg-blue-50/40 p-4 rounded-xl border border-blue-100/50">
